(function($) {
	...
	function switchAction(id, open) {
		...
		if(open) {
			$(".select2-results__options li[val=" + id + "] span[class]:eq(0)").removeClass("fa-toggle-right").addClass("fa-toggle-down");
			$(".select2-results__options li[parent='" + id + "']").slideDown();
		} else {
			$(".select2-results__options li[val=" + id + "] span[class]:eq(0)").addClass("fa-toggle-right").removeClass("fa-toggle-down");
	...
	function open() {
		setTimeout(function() {
			...
			$(".select2-results__options li").each(function() {
				...
				if($this.attr("parent")) {
					$(this).siblings("li[val=" + $this.attr("parent") + "]").find("span:eq(0)").addClass("fa fa-toggle-down switch").css({
						"padding": "0 10px",
						"cursor": "default"
					});
					// $(this).siblings("li[val=" + $this.attr("parent") + "]").find("span:eq(1)").css("font-weight", "bold");
					$(this).siblings("li[val=" + $this.attr("parent") + "]").find("span:eq(1)");
				}
			...
			$(".switch").mousedown(function() {
				switchAction($(this).parent().attr("val"), $(this).hasClass("fa-toggle-right"));
