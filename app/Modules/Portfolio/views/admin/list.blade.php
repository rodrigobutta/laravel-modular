...
@section('content')
    ...
    <div class="row">
        <div class="col-md-8">
            <h2 id="item_title">Elementos del Portfolio</h2>
        </div>
        <div class="col-md-4">
            <div class="pull-right">
                <button class="btn btn-primary" data-toggle="modal" data-target="#modalCreate">Nuevo</button>
                {{--  <a href="{{ route('admin.portfolio.import') }}" class="btn btn-primary export-to-excel pull-right" style="margin: 0 0 0 20px;"><i class="fa fa-upload"></i> Importar Excel</a>&nbsp;  --}}
                {{--  <a href="{{ route('admin.portfolio.export') }}" class="btn btn-primary export-to-excel pull-right"><i class="fa fa-download"></i> Exportar a Excel</a>  --}}
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="modalCreate" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                {!! Form::open(['method' => 'PUT', 'route' => 'admin.portfolio.create', 'role' => 'form'])  !!}
...
@section('extra-js')
    ...
    <script>
        $(function() {
            ...
            $('#portfolio-table').on('click','.btn[rel="delete"]',function(e) {
                e.preventDefault();
            ...
            $('#portfolio-table').DataTable({
                ...
                order: [[ 1, "desc" ]],
                ajax: '{!! route('admin.portfolio.data', ['type' => $type]) !!}',
                columns: [
                    { data: 'thumbnail', name: 'thumbnail', orderable: false, searchable: false },
                    { data: 'title', name: 'portfolio.title' },
                    { data: 'job', name: 'portfolio.job' },
        ...
        function makeSortable(){
            ...
            $("#portfolio-table tbody").sortable({
...
//                    console.log(ordered_ids)
                    $.ajax({
                        type: "POST",
                        url: "{{ url('admin/portfolio/reorder') }}",
                        data: {
                            order: ordered_ids
                        },
                        globalLoading: true
